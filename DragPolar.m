...
if Plot_DragPolar_Data == 1
    ...
    for n=1:Count
        ...
        legend({
            'Airfoil Drag Polar', 
            'Wing Drag Polar', 
            %sprintf('Drag Polar- %s', OswaldModel_Names.OswaldModel1(1)), 
            sprintf('Aircraft Drag Polar- %s', OswaldModel_Names.OswaldModel2(1)), 
            %sprintf('Drag Polar- %s', OswaldModel_Names.OswaldModel3(1)),
            %'Benchmark',
        }, 'Location', 'northwest');
    ...
    %     xlabel('Coefficient of Lift (CL)');
    %     ylabel('Coefficient of Drag (CD)');
    %     title('Drag Polar Configuration Comparison -', OswaldModel_Names.OswaldModel1(1));
    %     grid on
    %     hold off
    % end
    ...
    for n = 1:Count
        ...
        xlabel('Coefficient of Lift (CL)');
        ylabel('Coefficient of Drag (CD)');
        title('Design Aircraft Drag Polar Configuration Comparison -', OswaldModel_Names.OswaldModel2(1));
        grid on
        hold off
    end
    legend(Design_Input.Config(:),'Location','northwest');
    ...
    %     xlabel('Coefficient of Lift (CL)');
    %     ylabel('Coefficient of Drag (CD)');
    %     title('Drag Polar Configuration Comparison -', OswaldModel_Names.OswaldModel3(1));
    %     grid on
    %     hold off
    % end
    % legend(Design_Input.Config(:),'Location','northwest');
